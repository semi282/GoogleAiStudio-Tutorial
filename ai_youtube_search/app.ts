/**
 * 자막뽑기 AI - Frontend Application Logic (TypeScript)
 * 
 * Engine:
...
export interface CostEstimate {
  ...
  total_cost_krw_formatted: string;
  pricing_rate: string;
}
...
// Global window declarations for inline HTML event handlers
declare global {
  interface Window {
    toggleLeftDrawer: () => void;
    toggleRightDrawer: () => void;
    openRightDrawer: () => void;
    switchRightTab: (tab: 'script' | 'chat' | 'summary') => void;
    openRightTab: (tab: 'script' | 'chat' | 'summary') => void;
    loadYouTubeVideo: (videoId: string, startSeconds?: number) => void;
    seekToTime: (seconds: number) => void;
    startDirectSearch: () => Promise<void>;
...
const API_BASE: string = (window.location.protocol.startsWith('http') && window.location.port)
  ? ''
  : 'http://127.0.0.1:8000';

let historyList: SessionData[] = JSON.parse(localStorage.getItem('zapok_history') || '[]');
...
export async function checkApiConfig(): Promise<void> {
  try {
    const res = await fetch(`${API_BASE}/api/config`);
    const data: ApiConfigResponse = await res.json();
    const badge = document.getElementById('apiKeyBadge') as HTMLElement | null;
    const notice = document.getElementById('modalEnvKeyNotice') as HTMLElement | null;

    if (badge && notice) {
      if (data.has_env_key) {
        badge.textContent = "API 연동됨";
        notice.textContent = "✔ 서버 환경변수(GEMINI_API_KEY)가 정상 감지되었습니다.";
      } else {
        badge.textContent = "API 키 입력";
...
export function openRightDrawer(): void {
  const drawer = document.getElementById('rightDrawer');
  if (!drawer) return;
  drawer.classList.remove('w-0', 'translate-x-full');
  drawer.classList.add('w-96');
...
export function switchRightTab(tab: 'script' | 'chat' | 'summary'): void {
  const tabs: Array<'script' | 'chat' | 'summary'> = ['script', 'chat', 'summary'];
  tabs.forEach(t => {
...
export function openRightTab(tab: 'script' | 'chat' | 'summary'): void {
  openRightDrawer();
  switchRightTab(tab);
}
...
export function loadYouTubeVideo(videoId: string, startSeconds: number = 0): void {
  const placeholder = document.getElementById('videoPlaceholder');
  const iframe = document.getElementById('videoIframe') as HTMLIFrameElement | null;
...
export async function startDirectSearch(): Promise<void> {
  const urlInput = document.getElementById('youtubeUrl') as HTMLInputElement | null;
...
export function renderSession(s: SessionData): void {
    ...
    if (s.transcript) {
      ...
      lines.forEach(line => {
        ...
        if (tsMatch) {
          ...
          const formattedLine = line.replace(/\[(\d{1,2}:\d{2}(?::\d{2})?)\]/g, (_match, p1) => {
...
export async function loadHistoryFromServer(): Promise<void> {
  try {
    ...
    if (res.ok) {
      const list: SessionData[] = await res.json();
...
export function openSettingsModal(): void {
  const modalInput = document.getElementById('modalApiKey') as HTMLInputElement | null;
...
// Attach functions to window object for HTML inline event handlers
...
window.openRightTab = openRightTab;
window.loadYouTubeVideo = loadYouTubeVideo;
window.seekToTime = seekToTime;
window.startDirectSearch = startDirectSearch;
